...
ВОЗМОЖНОСТИ:
• Исключительно модель 'gemini-3.6-flash' — без перебора устаревших моделей.
...
================================================================================
ВСТАВЬТЕ СВОЙ АПИ КЛЮЧ В GOOGLE_API_KEY=YOUR_API_KEY ниже в коде
"""
...
# Опциональный захват звука (динамики / WASAPI Loopback / микрофон)
try:
...
class Config:
    ...
    # Исключительно актуальная модель Gemini 3.6 Flash
    MODEL_NAME = "gemini-3.6-flash"

    # Глобальные сочетания клавиш (через CTRL + ALT)
    CAPTURE_HOTKEY = "ctrl+alt+s"       # Захват и анализ экрана браузера
    AUDIO_HOTKEY = "ctrl+alt+q"         # Старт/Стоп прослушки динамиков (голос собеседника в Zoom / Телемост)
    HIDE_HOTKEY = "ctrl+alt+c"          # Скрыть оверлей
    SHOW_HOTKEY = "ctrl+alt+d"          # Открыть оверлей с предыдущим ответом
    SCROLL_UP_HOTKEY = "ctrl+alt+z"     # Прокрутка ответа ВВЕРХ
    SCROLL_DOWN_HOTKEY = "ctrl+alt+x"   # Прокрутка ответа ВНИЗ

    # Системный промпт: строжайшее правило для задач по кодингу — только чистый Python-код
    SYSTEM_PROMPT = (
        "Ты — персональный встроенный ассистент разработчика. "
        "Внимательно изучи приложенный скриншот экрана браузера, найди главный вопрос или задачу. "
    ...
    # Системный промпт для аудиовопросов собеседника (из Zoom, Яндекс Телемоста, Teams)
    AUDIO_PROMPT = (
        "Ты — персональный тайный ассистент на техническом интервью или встрече в Zoom / Яндекс Телемосте. "
        "Тебе передана аудиозапись речи собеседника или интервьюера из динамиков. "
        "Дай ТОЧНЫЙ, ЧЕТКИЙ, БЕЗ ВОДЫ ответ на вопрос собеседника. "
        "Если вопрос по кодингу или архитектуре — дай сразу работающий код на Python 3 или четкую структуру тезисов без общих вступительных фраз и воды."
    )
    ...
    OVERLAY_HEIGHT = 340
    CORNER_MARGIN = 24
    SCROLL_STEP = 90         # Шаг прокрутки в пикселях при нажатии горячих клавиш
    AUTO_HIDE_SECONDS = 0    # 0 = не скрывать автоматически
...
class WindowCapture:
    ...
    def capture_window_from_memory(hwnd: int) -> Optional[Image.Image]:
        ...
        try:
            ...
            # PW_RENDERFULLCONTENT = 2 (DirectX / Chromium Hardware Acceleration)
...
class AudioLoopbackRecorder:
    """
    Записывает входящий звук (голос собеседника из Zoom, Яндекс Телемоста, Teams, браузера).
    Использует WASAPI Loopback (захват исходящего аудиопотока Windows) с автоматическим
    fallback на стандартный микрофон, если WASAPI Loopback не настроен в ОС.
    """
    ...
    def start(self) -> bool:
        ...
        # Поиск устройства WASAPI Loopback (динамики / наушники Windows)
        ...
        try:
            # Попытка открыть поток loopback на динамики
            if loopback_device is not None:
                try:
                    ...
                    self._stream.start()
                    print(f"[Audio] Запись звука начата через WASAPI Loopback (устройство #{loopback_device}).")
                    return True
                except Exception as loop_err:
                    print(f"[Audio] Loopback не удался ({loop_err}), переключаемся на стандартный ввод...")

            # Fallback на системный микрофон / стерео микшер
            self._stream = sd.InputStream(
                samplerate=self.samplerate,
                channels=1,
                dtype="int16",
    ...
    def stop(self) -> Optional[bytes]:
        ...
        with self._lock:
            if not self.audio_frames:
                print("[Audio] Аудиопоток пуст (0 фреймов).")
                return None
            try:
        ...
        try:
            ...
            print(f"[Audio] Запись завершена. Размер WAV: {len(wav_bytes)} байт (~{len(audio_data)/self.samplerate:.1f} сек).")
            return wav_bytes
        except Exception as e:
...
class GeminiWorker(QObject):
    ...
    def _execute_analysis(self, image: Image.Image, window_title: str):
        ...
        try:
            ...
            print(f"[AI] Отправка скриншота строго в модель '{self.config.MODEL_NAME}'...")
            ...
            if not response or not response.text:
                raise RuntimeError("Модель gemini-3.6-flash вернула пустой ответ.")
    ...
    def _execute_audio_analysis(self, audio_wav_bytes: bytes):
        ...
        try:
            ...
            # Очистка Markdown-блоков при необходимости
            ...
            error_msg = str(err)
            print(f"[AI АУДИО ОШИБКА] {error_msg}")
            self.error_occurred.emit(f"Ошибка аудио-анализа: {error_msg}")
...
class HUDOverlay(QWidget):
    """
    Прозрачный, сквозной (Click-Through) HUD-оверлей поверх всех окон.
    Все клики мыши проходят сквозь окно в браузер под ним.
    Ответ сохраняется в памяти и может быть повторно показан по Alt+Shift+D.
    """
    ...
    def _init_ui(self):
        ...
        # Бейдж скрытия от Zoom / Яндекс Телемост / Teams / OBS
        ...
        # Текстовое поле с ответом / чистым кодом
        ...
        # Нижняя подсказка по скроллу, скрытию, аудио и открытию
    ...
    def _apply_stealth_and_click_through(self):
        """
        Применяет системные Win32 атрибуты окна:
        1. WS_EX_TRANSPARENT — сквозной клик (мышь свободно кликает сквозь HUD в элементы браузера).
        2. SetWindowDisplayAffinity(WDA_EXCLUDEFROMCAPTURE = 0x00000011) —
           ПОЛНАЯ НЕВИДИМОСТЬ для программ демонстрации и записи экрана:
           Zoom, Яндекс Телемост, Microsoft Teams, Discord, Google Meet, OBS Studio, Skype.
           Окно отображается ТОЛЬКО на вашем физическом мониторе, а в поток захвата экрана
           DWM передает чистое изображение рабочего стола без HUD.
        """
        try:
            hwnd = int(self.winId())

            # 1. Сквозной клик мыши
            ...
            # 2. Невидимость для Zoom, Яндекс Телемост, Teams, OBS (SetWindowDisplayAffinity)
            if getattr(self.config, "HIDE_FROM_SCREEN_CAPTURE", True):
                # 0x00000011 = WDA_EXCLUDEFROMCAPTURE (Windows 10 v2004+ и Windows 11)
                WDA_EXCLUDEFROMCAPTURE = 0x00000011
                res = ctypes.windll.user32.SetWindowDisplayAffinity(hwnd, WDA_EXCLUDEFROMCAPTURE)
                if res == 0:
                    # Fallback для устаревших сборок Windows
                    WDA_MONITOR = 0x00000001
                    ctypes.windll.user32.SetWindowDisplayAffinity(hwnd, WDA_MONITOR)
        except Exception as e:
            print(f"[HUDOverlay] Ошибка применения системных атрибутов окна: {e}")
    ...
    def scroll_up(self):
        """Прокрутка текста вверх по горячей клавише Ctrl + Alt + Z."""
        if self.isVisible():
            bar = self.text_area.verticalScrollBar()
            bar.setValue(bar.value() - self.config.SCROLL_STEP)

    def scroll_down(self):
        """Прокрутка текста вниз по горячей клавише Ctrl + Alt + X."""
        if self.isVisible():
            bar = self.text_area.verticalScrollBar()
            bar.setValue(bar.value() + self.config.SCROLL_STEP)

    def show_recording_started(self):
        """Индикация включения записи голоса собеседника из динамиков."""
        ...
        self.text_area.setHtml(
            ...
            f"Когда собеседник договорит вопрос — нажмите <b>{self.config.AUDIO_HOTKEY.upper()}</b> еще раз.<br>"
            "Запись мгновенно остановится и отправится на сервер Gemini для точного и четкого ответа без воды."
            "</p>"
            "</div>"
        )
        self.show()
    ...
    def show_restored_response(self):
        """Показ сохраненного ответа по Ctrl + Alt + D (после закрытия по Ctrl + Alt + C)."""
        print(f"[HUD] Восстановление предыдущего ответа по {self.config.SHOW_HOTKEY.upper()}...")
        self.reposition_to_corner()
        self.status_dot.setText("●")
        self.status_dot.setStyleSheet("color: #38bdf8; font-size: 13px; background: transparent; border: none;")
    ...
    def show_thinking(self, window_title: str):
        ...
        self.text_area.setHtml(
            ...
            "Запрос отправлен в " + self.config.MODEL_NAME + "... Генерация точного ответа без воды.<br>"
            "Фокус ввода сохранен, окно прозрачно для кликов."
            "</p>"
        )
    ...
    def show_response(self, text: str):
        # Сохраняем ответ в память для повторного открытия по Ctrl + Alt + D
        ...
        # Сбрасываем скролл в начало текста
...
class ApplicationController:
    ...
    def _handle_audio_toggle(self):
        """
        Обработка нажатия клавиши CTRL + ALT + Q:
        1-е нажатие: открывается окно оверлея и начинается захват исходящего звука (голос собеседника).
        2-е нажатие: запись завершается, аудио сразу отправляется на сервер Gemini, в окно выводится ответ без воды.
        """
...
def main():
    ...
    print("Правило генерации: ТОЛЬКО чистый код Python 3 или четкий ответ без воды.")
